...
public class DynamicTask {
        ...
        if (tasks.size() > 0) {
            for (TSTimeTaskEntity task : tasks) {
                ...
                try {
                    ...
                    String runServerIp = task.getRunServerIp();
                    if (ipList.contains(runServerIp) || StringUtil.isEmpty(runServerIp) || "本地".equals(runServerIp)) {//当前服务器IP匹配成功
                        //只加载后台需要的com.fnjz.front.timer目录下的定时任务
                        if (StringUtils.contains(task.getClassName(), "front.timer")) {
                            //quartz 1.6
                            JobDetail jobDetail = new JobDetail();
                            jobDetail.setName(task.getId());
                            jobDetail.setGroup(Scheduler.DEFAULT_GROUP);
    ...
    @Test
    public void run() {
        System.out.println(StringUtils.contains("com.fnjz.front.timer.AccountNotify", "front.timer"));
    }
}

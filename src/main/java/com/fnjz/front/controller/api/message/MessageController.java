...
public class MessageController extends BaseController {
    ...
    public ResultBean messageList(HttpServletRequest request, String userId, Integer page, Integer rows) {

        if (StringUtils.isEmpty(userId)) {
            userId = (String) request.getAttribute("userInfoId");
        }
        if (page == null) {
            page = 1;
        }
    ...
    public ResultBean updateMessageStatus(HttpServletRequest request,@RequestBody JSONObject jsonObject) {
        ...
        if (StringUtils.isEmpty(userinfoId)){
            userinfoId = (String) request.getAttribute("userInfoId");
        }

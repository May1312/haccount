...
public class UserCommUseIncomeRestServiceImpl extends CommonServiceImpl implements UserCommUseIncomeRestServiceI {
    ...
    public Map<String, Object> getListById(String user_info_id) throws InvocationTargetException, IllegalAccessException {
        ...
        List<IncomeTypeRestEntity> allList = new ArrayList();
        List<IncomeTypeRestEntity> commonList = new ArrayList();
        Map<String, Object> map = new HashMap();
        if (list.isEmpty()) {
            //用户常用表为null，返回系统常用
            String hql2 = "FROM IncomeTypeRestEntity where status = 1 ORDER BY priority ASC";
            List<IncomeTypeRestEntity> list2 = commonDao.findByQueryString(hql2);
            if (!list2.isEmpty()) {
                //组合二三级类目 获取所有三级类目
                for (int i = 0; i < list2.size(); i++) {
                    if (StringUtils.isEmpty(list2.get(i).getParentId())) {
                        IncomeTypeRestEntity income1 = new IncomeTypeRestEntity();
                        ...
                        for (int j = 0; j < list2.size(); j++) {
                            if (StringUtils.equals(list2.get(i).getId(), list2.get(j).getParentId())) {
                                //封装三级类目
                                IncomeTypeRestEntity jIncome = new IncomeTypeRestEntity();
                                ...
                                if (StringUtils.isNotEmpty(list2.get(j).getParentId())) {
                                    if (list2.get(j).getMark() == 1) {
                                        if (flag) {
                                            IncomeTypeRestEntity income2 = new IncomeTypeRestEntity();
                                            BeanUtils.copyProperties(list2.get(i), income2, new String[]{"IncomeTypeSons"});
                                            commonList.add(income2);
                                            //获取当前角标
                                            int index2 = commonList.size() - 1;
                                            IncomeTypeRestEntity income3 = new IncomeTypeRestEntity();
                                            BeanUtils.copyProperties(list2.get(j),income3);
                                            commonList.get(index2).getIncomeTypeSons().add(income3);
                                            flag = false;
                                        } else {
                                            int index2 = commonList.size() - 1;
                                            IncomeTypeRestEntity income4 = new IncomeTypeRestEntity();
                                            BeanUtils.copyProperties(list2.get(i), income4, new String[]{"IncomeTypeSons"});

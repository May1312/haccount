...
public interface FengFengTicketRestDao {
    ...
    @Sql("SELECT cycle,behavior_ticket_value as cycleAware,UNIX_TIMESTAMP(downtime)*1000 as downtime FROM `hbird_fengfeng_ticket` where category_of_behavior = :categoryOfBehavior and acquisition_mode=:acquisitionMode and status=1 and if(downtime is null,1=1,downtime>=CURRENT_TIMESTAMP) and cycle!=1 order by cycle;")

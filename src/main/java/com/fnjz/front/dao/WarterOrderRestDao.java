...
public interface WarterOrderRestDao {
    ...
    @Sql("SELECT base1.*,base3.nick_name as reporter_nick_name,base3.avatar_url as reporter_avatar,base2.ab_name as ab_name,base2.account_book_type_id as abTypeId FROM hbird_water_order AS base1 INNER JOIN hbird_user_info AS base3 ON base1.update_by = base3.id, ( SELECT base2.id, base2.ab_name FROM ( SELECT account_book_id,account_book_type_id FROM hbird_user_account_book WHERE user_info_id = :userInfoId ) AS base1, hbird_account_book AS base2 WHERE base2.id = base1.account_book_id AND base2.STATUS = 0 ) AS base2 WHERE base1.account_book_id = base2.id AND if(:synDate is null,1=1,base1.update_date>:synDate);")
     ...
     */
    @ResultType(APPWarterOrderRestDTO.class)
    @Sql("SELECT base1.*,base3.nick_name as reporter_nick_name,base3.avatar_url as reporter_avatar,base2.ab_name as ab_name,base2.account_book_type_id as abTypeId FROM hbird_water_order AS base1 INNER JOIN hbird_user_info AS base3 ON base1.update_by = base3.id, ( SELECT base2.id, base2.ab_name FROM ( SELECT account_book_id,account_book_type_id FROM hbird_user_account_book WHERE user_info_id = :userInfoId and delflag=0 ) AS base1, hbird_account_book AS base2 WHERE base2.id = base1.account_book_id AND base2.STATUS = 0 ) AS base2 WHERE base1.account_book_id = base2.id AND if(:synDate is null,1=1,base1.update_date>:synDate) and base1.delflag=0;")

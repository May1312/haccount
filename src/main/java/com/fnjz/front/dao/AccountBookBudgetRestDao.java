...
public interface AccountBookBudgetRestDao {
    ...
    @Sql("SELECT CASE WHEN righta.time IS NULL THEN rightb.time ELSE righta.time END AS time, CASE WHEN righta.budgetMoney IS NULL THEN rightb.budgetMoney ELSE righta.budgetMoney END AS budgetMoney, CASE WHEN righta.fixedLargeExpenditure IS NULL THEN rightb.fixedLargeExpenditure ELSE righta.fixedLargeExpenditure END AS fixedLargeExpenditure, CASE WHEN righta.fixedLifeExpenditure IS NULL THEN rightb.fixedLifeExpenditure ELSE righta.fixedLifeExpenditure END AS fixedLifeExpenditure, CASE WHEN righta.monthSpend IS NULL THEN rightb.monthSpend ELSE righta.monthSpend END AS monthSpend, CASE WHEN righta.monthIncome IS NULL THEN rightb.monthIncome ELSE righta.monthIncome END AS monthIncome FROM ( SELECT SUM( CASE WHEN order_type = 1 THEN money ELSE 0 END ) AS monthSpend, SUM( CASE WHEN order_type = 2 THEN money ELSE 0 END ) AS monthIncome, DATE_FORMAT( charge_date, '%Y-%m' ) AS time, budget.budget_money AS budgetMoney, budget.fixed_large_expenditure AS fixedLargeExpenditure, budget.fixed_life_expenditure AS fixedLifeExpenditure FROM `hbird_water_order` AS wo INNER JOIN ( SELECT time, budget_money, fixed_large_expenditure, fixed_life_expenditure FROM `hbird_accountbook_budget` WHERE account_book_id = :accountBookId AND time <= CONCAT( DATE_FORMAT( NOW( ), '%Y-' ), :month ) AND time >= :rangeMonth AND ( fixed_large_expenditure IS NOT NULL OR fixed_life_expenditure IS NOT NULL ) AND ( fixed_large_expenditure !=- 1 OR fixed_life_expenditure !=- 1 ) ) AS budget ON wo.account_book_id = :accountBookId AND wo.delflag = 0 GROUP BY time ) AS righta, ( SELECT SUM( CASE WHEN order_type = 1 THEN money ELSE 0 END ) AS monthSpend, SUM( CASE WHEN order_type = 2 THEN money ELSE 0 END ) AS monthIncome, DATE_FORMAT( charge_date, '%Y-%m' ) AS time, budget.budget_money AS budgetMoney, budget.fixed_large_expenditure AS fixedLargeExpenditure, budget.fixed_life_expenditure AS fixedLifeExpenditure FROM `hbird_water_order` AS wo INNER JOIN ( SELECT time, budget_money, fixed_large_expenditure, fixed_life_expenditure FROM `hbird_accountbook_budget` WHERE account_book_id = :accountBookId AND time <= CONCAT( DATE_FORMAT( NOW( ), '%Y-' ), :month ) AND time >= :rangeMonth AND ( fixed_large_expenditure IS NOT NULL OR fixed_life_expenditure IS NOT NULL ) AND ( fixed_large_expenditure !=- 1 OR fixed_life_expenditure !=- 1 ) ) AS budget ON wo.account_book_id = :accountBookId AND wo.delflag = 0 GROUP BY time ) AS rightb;")

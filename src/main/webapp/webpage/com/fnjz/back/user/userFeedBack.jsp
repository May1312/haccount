					...
					<td class="value">
						<%--<input class="inputxt" id="status" name="status" ignore="ignore"  value="${userFeedBackPage.status}" />:0_未处理   1_已查看  2_已解决:--%>
							<select class="selectxt" id="status" name="status" ignore="ignore"
								...
								<option value="0" <c:if test="${'0' == userFeedBackPage.status}">selected="selected"</c:if> >未处理</option>
								<option value="1" <c:if test="${'1' == userFeedBackPage.status}">selected="selected"</c:if>>已查看 </option>
								<option value="2" <c:if test="${'2' == userFeedBackPage.status}">selected="selected"</c:if>>已解决</option>
